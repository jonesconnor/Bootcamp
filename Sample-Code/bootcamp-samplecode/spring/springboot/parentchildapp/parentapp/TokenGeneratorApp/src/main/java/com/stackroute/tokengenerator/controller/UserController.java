...
public class UserController {
	...
	public ResponseEntity<?> getbymail(@PathVariable("name") String name)
	{
	UserProfile obj=	uservice.getUserbyName(name);
		return new ResponseEntity<UserProfile>(obj,HttpStatus.OK);
	}
	
	@DeleteMapping("/delete/name/{name}")
	public ResponseEntity<?> delete(@PathVariable("name") String name)
	{
boolean result=	uservice.deleteByName(name);

   if (result)
		return new ResponseEntity<String>("Deeted successfully",HttpStatus.OK);
   else
	   return new ResponseEntity<String>("Name not found ",HttpStatus.OK);
	}
}
